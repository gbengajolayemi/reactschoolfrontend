...
const SS3StudentsResult = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/results/ss3');
  ...
  const handleSaveEdit = async (studentId) => {
    try {
      await axios.put(`http://localhost:5000/api/results/ss3/ss3students/${studentId}`, editedResultData);
  ...
  const handleDelete = async (studentId) => {
    if (window.confirm('Are you sure you want to delete this student result?')) {
      try {
        await axios.delete(`http://localhost:5000/api/results/ss3/ss3students/${studentId}`);
  ...
  const handleAddResultSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/results/ss3/uploadSingleResult', newResultData);
...
export default SS3StudentsResult;
